...
export function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    const payload = { email, password, profileCreatedBy, gender };
  console.log("1. FRONTEND SUBMITTING PAYLOAD:", payload); 
  ...
  // 2. Phone Regex Validation (Matches standard 10-digit formats with optional country code)
  const phoneRegex = /^\+?[1-9]\d{1,14}$/; // International E.164 phone standard format
  if (!phoneRegex.test(phone)) {
    ...
    try {
      const response = await fetch('http://localhost:3000/api/auth/register', {
        ...
        body: JSON.stringify({ email, password, phone, profileCreatedBy, gender }),
      });
       console.log("2. SERVER RESPONSE STATUS:", response.status); 
      const data = await response.json();
      console.log("3. SERVER RESPONSE DATA:", data);
      if (!response.ok) throw new Error(data.message || 'Registration failed');
      navigate('/login');
    } catch (err: any) {
       console.error("4. FETCH CATCH BLOCK ERROR:", err); 
      setError(err.message);
    }
  };

  return (
    <div className="flex min-h-[85vh] justify-center items-center bg-gray-100 p-4">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div className="space-y-3">
            <div>
              ...
              <select className="w-full mt-1 p-2.5 border border-gray-200 rounded-lg bg-white text-sm focus:ring-2 focus:ring-pink-500 outline-none" value={profileCreatedBy} onChange={(e) => setProfileCreatedBy(e.target.value)} >
            ...
            <div>
  <label className="block text-xs font-semibold text-gray-600">Mobile Phone Number</label>
  <input 
    type="tel" 
    required 
    className="w-full mt-1 p-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-pink-500 outline-none" 
