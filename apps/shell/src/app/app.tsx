...
export function App() {
  ...
  // 1. Read initial token state directly from localStorage
  const [isLoggedIn, setIsLoggedIn] = useState(!!localStorage.getItem('token'));

  // 2. Watch for route changes to keep the auth state synchronized
  useEffect(() => {
      ...
      <div className="flex-1 flex flex-col lg:flex-row w-full max-w-7xl mx-auto px-4 lg:px-8 py-6 gap-8 items-start justify-center">
        ...
        <main 
          className={`w-full transition-all duration-500 ${
            showRegisterSidePanel ? 'lg:w-2/3 pointer-events-none select-none' : 'w-full'
          }`}
        >
